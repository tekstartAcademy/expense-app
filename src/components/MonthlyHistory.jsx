    ...
    <Box>
      ...
      {monthsHistory && monthsHistory.length > 0 ? (
        monthsHistory
          ...
          .map((month) => (
            <Card key={month.id} sx={{ marginBottom: "20px", backgroundColor: "background.paper" }}>
              <CardContent>
                ...
                <Typography color="text.primary">
                  Total Expenses: ${month.bills ? month.bills.reduce((sum, bill) => sum + bill.amount, 0).toFixed(2) : "0.00"}
                </Typography>
                <Typography color="text.primary">
                  Remaining Balance: ${month.remainingBalance !== undefined ? month.remainingBalance.toFixed(2) : "0.00"}
                </Typography>
